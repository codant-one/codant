                ...
                <table id="kt_datatable_example_1" class="table align-middle table-row-bordered fs-6" style="width:100%">
                    <thead class="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
                        <tr>
                            ...
                            <th class="w-200px">TÍTULO (ES)</th>
                            <th class="w-150px">CATEGORÍA</th>
                            <th class="w-100px">AÑO</th>
                            <th class="w-100px">POPULAR</th>
                            <th class="w-150px">USUARIO</th>
                            @canany(['blog_edit', 'blog_delete'])
                        ...
                        <tr class="bg-white">
                            ...
                            <th>
                                <input type="text" class="form-control form-control-sm" placeholder="TÍTULO ESPAÑOL">
                            </th>
                            <th>
                                <input type="text" class="form-control form-control-sm" placeholder="CATEGORÍA">
                            </th>
                            <th>
...
@php
    $edit_route = route("blogs.edit", ["blog" => 'id-here']);
    $delete_route = route("blogs.destroy", ["blog" => 'id-here']);
@endphp

<script>
    $(document).ready(function () {
     
        var can = "{!!  auth()->user()->canany(['blog_edit','blog_delete']) !!}";
            ...
            columnDefs: [
                {
                    ...
                    render: function (data, type, row) {
                        const title = data && data.length > 30 ? `${data.slice(0, 30).replace(/\s+$/, '')}...` : data;

                        return `<a href="${ `{{ route("blogs.edit", ["blog" => 'here']) }}`.replace('here', row.id) }" class="text-dark fw-bolder text-hover-primary fs-6">
                                    ${title}
                                </a>`;
                    }
                },
                {
                    targets: 2,
                    render: function (data, type, row) {
                        return `<span class="badge badge-light-primary fs-7">${data}</span>`;
                    }
                },
                {
                    targets: 3,
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        return data ? 
                            '<span class="badge badge-success fs-7">Sí</span>' : 
                            '<span class="badge badge-light fs-7">No</span>';
                    }
                },
                {
                    targets: 5,
