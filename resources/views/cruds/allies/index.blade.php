...
@php
    $edit_route = route("allies.edit", ["ally" => 'id-here']);
    $delete_route = route("allies.destroy", ["ally" => 'id-here']);
@endphp

<script>
    $(document).ready(function () {
     
        var can = "{!!  auth()->user()->canany(['ally_edit','ally_delete']) !!}";
                    ...
                    render: function (data, type, row) {
                        ...
                        return `<div class="d-flex align-items-center">
                                        <div class="symbol symbol-circle symbol-40px overflow-hidden me-3">
                                            <a href="${ `{{ route("allies.edit", ["ally" => 'here']) }}`.replace('here', row.id) }">
                                                <div class="symbol-label">
                                                    <img src="${avatarUrl}" alt="ally-${row.id}" class="w-100"/>  
                                                </div>
                                            </a>
                                        </div>
                                        <div class="d-flex flex-column">
                                            <a href="${`{{ route("allies.edit", ["ally" => 'here']) }}`.replace('here', row.id)}" class="text-gray-800 text-hover-primary">
                                                 ${row.fullname}
                                            </a>
                                            <span>${row.email}</span>
                                        </div>
